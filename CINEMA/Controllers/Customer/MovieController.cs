...
namespace CINEMA.Controllers
{
    public class MovieController : AdminBaseController
    {
        ...
        {
            // 💡 Tự động Seed 20 phim đang chiếu & 10 phim sắp chiếu vào DB nếu chưa có
            CINEMA.Helpers.MovieSeeder.Seed(_context);

            var today = DateOnly.FromDateTime(DateTime.Today);

            // Tự động kiểm tra và chuyển các phim có EndDate < hôm nay thành IsActive = false (Tạm ngừng chiếu)
            var expiredMovies = _context.Movies
                .Where(m => m.IsActive == true && m.EndDate.HasValue && m.EndDate.Value < today)
                .ToList();

            if (expiredMovies.Any())
            {
                foreach (var m in expiredMovies)
                {
                    m.IsActive = false;
                }
            ...
            // Lấy tất cả các phim (bao gồm cả đang chiếu, sắp chiếu và đã ngừng chiếu)
        ...
        public IActionResult Edit(Movie updatedMovie, IFormFile? PosterImage)
        {
            ...
            // 💡 Kiểm tra Ngày kết thúc chiếu: Nếu EndDate đã qua (< hôm nay), tự động chuyển thành Tạm Ngừng Chiếu
            var today = DateOnly.FromDateTime(DateTime.Today);
            if (updatedMovie.EndDate.HasValue && updatedMovie.EndDate.Value < today)
            {
                updatedMovie.IsActive = false;
            }
