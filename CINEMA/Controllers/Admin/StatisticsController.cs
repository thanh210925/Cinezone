    ...
    public class StatisticsController : AdminBaseController
    {
        ...
        public async Task<IActionResult> ExportExcel(DateTime? from, DateTime? to)
        {
            ...
            var orders = await _context.Orders
                ...
                .Where(o => o.CreatedAt >= startDate && o.CreatedAt <= endDate && (o.Status == "Đã thanh toán" || o.Status == "Đã check-in"))
                .OrderByDescending(o => o.CreatedAt)
                .ToListAsync();

            var builder = new System.Text.StringBuilder();
            builder.AppendLine("MaDonHang,KhachHang,NgayTao,Phim,SoLuongVe,TienVe,TienCombo,GiamGia,TongTien,PhuongThucThanhToan,TrangThai");

            foreach (var o in orders)
            {
                string customerName = (o.Customer?.FullName ?? "Khach vang lai").Replace(",", " ");
                string movieTitle = (o.Tickets.FirstOrDefault()?.Showtime?.Movie?.Title ?? "N/A").Replace(",", " ");
                string createdAt = o.CreatedAt?.ToString("dd/MM/yyyy HH:mm") ?? "";
                int ticketCount = o.Tickets.Count;

                builder.AppendLine($"{o.OrderId},{customerName},{createdAt},{movieTitle},{ticketCount},{o.TicketTotal ?? 0},{o.ComboTotal ?? 0},{o.DiscountAmount ?? 0},{o.TotalAmount ?? 0},{o.PaymentMethod},{o.Status}");
            }

            byte[] buffer = System.Text.Encoding.UTF8.GetPreamble().Concat(System.Text.Encoding.UTF8.GetBytes(builder.ToString())).ToArray();
            return File(buffer, "text/csv; charset=utf-8", $"BaoCaoDoanhThu_{startDate:yyyyMMdd}_{endDate:yyyyMMdd}.csv");
        }
    }
}
